...
$rt = mysqli_query($con,"SELECT * FROM complaint where customer_id='".$_SESSION['customer_id']."' and status='not process yet' ");
...
{?>
                  ...
                  <span class="li_news"></span>
                    <?php 
  $status="in process";                   
$rt = mysqli_query($con,"SELECT * FROM complaint where customer_id='".$_SESSION['customer_id']."' and  status='$status'");
$num1 = mysqli_num_rows($rt);
{?>
                  <h3><?php echo htmlentities($num1);?></h3>
                        </div>
